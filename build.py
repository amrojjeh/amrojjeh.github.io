...
class MyExtensions(Extension):
	def extendMarkdown(self, md):
		md.postprocessors.register(BoilerplateProcessor(), "bp", 1)
	...
	for file in files:
		if file.endswith(".md"):
			...
			with open(file_path, "r", encoding="utf-8") as md_file:
				with open(join(root, file.replace(".md", ".html")), "w+", encoding="utf-8") as html_file:
					extension_configs = {
						"codehilite": {
							"guess_lang": False,
						},
					}
					html_file.write(markdown.markdown(md_file.read(), extensions=[MyExtensions(), "codehilite", "tables"], extension_configs=extension_configs, output_format="html5"))
